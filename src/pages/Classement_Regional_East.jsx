...
export default function ClassementRegionalEast({ tablesEast = [] }) {
  const [hoverIndex, setHoverIndex] = useState(null);
  const [selectedTeam, setSelectedTeam] = useState(null);

  const getMatchsForTeam = (team) => {
    const matchs = [];
    tablesEast.forEach(({ data }) => {
      data.forEach((row) => {
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>Classement Regional East</h2>
      {getClassement(tablesEast).length === 0 ? (
        ...
        <table style={styles.table}>
          ...
          <tbody style={styles.tbody}>
            {getClassement(tablesEast).map((team, idx) => {
              const isOdd = idx % 2 === 1;
              return (
                <tr
                  key={team.team}
